...
def to_char_dataset(samples: list) -> (TensorDataset, dict):
    ...
    char_dataset = TensorDataset(sent_ids, token_idx, token_input_ids, token_attention_mask, char_token_idx,
                                 char_input_ids, char_attention_mask, char_label_ids)
    id2sample = {sample['sent_idx']: sample for sample in samples}
    return char_dataset, id2sample


def run_char_step(device, batch: tuple, cx_model: CharXfmrNerModel,
                  model_optimizer: ModelOptimizer) -> (torch.Tensor, torch.Tensor):
    batch = tuple(t.to(device) for t in batch)
    loss, pred_char_labels = cx_model(*batch)
    if model_optimizer is not None:
        loss.backward()
        model_optimizer.step()
    return loss, pred_char_labels.cpu()
...
def decode_char_batch(batch: list, samples: dict, cx_model: CharXfmrNerModel) -> (list, list):
    ...
    for i in range(char_input_ids.size(0)):
        sent_id = sent_ids[i]
        sent_data_sample = samples[sent_id.item()]
        sent_attention_mask = char_attention_mask[i]
        sent_gold_ids = char_label_ids[i][sent_attention_mask == 1]
        sent_pred_ids = char_prediction_ids[i][sent_attention_mask == 1]
...
def run_char(epoch, phase, device, data: DataLoader, samples: dict, cx_model: CharXfmrNerModel, print_every,
    ...
    for i, batch in enumerate(data):
        step = i + 1
        batch_loss, batch_predictions = run_char_step(device, batch[2:], cx_model, model_optimizer)
        gold_char_labeled_sentences, pred_char_labeled_sentences = decode_char_batch(batch + [batch_predictions],
                                                                                     samples, cx_model)
