...
def main():
    ...
    if ner_model_type == 'char':
        ...
        for project_type in ['news', 'fin']:
            ...
            dataset_sentences[project_type] = labeled_sentences
        ...
        for project_type in ['news', 'fin']:
            labeled_sentences = dataset_sentences[project_type]
        ...
        for data_type in ['train', 'eval']:
            annotated_sentences = adm.read_project_sentences(Path(f'data/clean/mer/MER-2020-02-11/{data_type}'))
            token_labeled_sentences = label_token_sentences(annotated_sentences)
            dataset_sentences[data_type] = token_labeled_sentences
        ner_model = create_token_ner_model(xfmr_model_type, 0.0, classifier_type)
        for data_type in ['train', 'eval']:
            labeled_sentences = dataset_sentences[data_type]
            dataset_name = f'{data_type}-{ner_model_type}-{xfmr_model_type}'
            data_file_path = Path(f'data/processed/{dataset_name}.csv')
            sample_file_path = Path(f'data/processed/{dataset_name}.pkl')
